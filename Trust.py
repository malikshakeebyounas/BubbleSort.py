...
def button_Search_result():
    ...
    # Empty grid Data before filling grid
    for row_idx, row_data in enumerate(old_rows, start=1):
        for col_idx, cell_data in enumerate(row_data):
            tk.Label(table, text='', width=15).grid(row=row_idx, column=col_idx)

    # Fill data in grid from database procedure [dbo].[Search]
    for row_idx, row_data in enumerate(rows, start=1):
        for col_idx, cell_data in enumerate(row_data):
            tk.Label(table, text=cell_data, width=15).grid(row=row_idx, column=col_idx)

    # Create Link with First Column of data in grid
    for row_idx, row_data in enumerate(rows, start=1):
        for col_idx, cell_data in enumerate(row_data):
...
labelAid_id = ttk.Label(page_NewEdit, text="Aid Id : ")
labelAid_id.grid(padx=0, pady=7)

tb_Aid_id = StringVar()
input_field = tkinter.Entry(page_NewEdit, textvariable=tb_Aid_id)
