...
while index<=20:
    result=index*num
    print(str(num) + " * " + str(index) + "=" + str(result))
    index=index + 1"""
...
def button_0_Clicked(buttons_array,loop,row,column):
    global size_window
...
def button_1_Clicked():
    ...
    for r in range(0, loop):
        ...
        for c in range(0, loop):
            ...
            button_0 = tkinter.Button(buttons_window, text=count, width=7, height=3, command = lambda row=r,column=c: button_0_Clicked(buttons_array,loop, row,column))
            #button_0.pack()
